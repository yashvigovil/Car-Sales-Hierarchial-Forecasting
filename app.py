...
# ----------------------------------------------------
    ...
    with tab_ingest:
        ...
        **Key Steps:**
        ...
        """)
        st.code("""
# Column normalization snippet from Ingestion.py
clean_columns = [
    col_name.strip()
    .replace(" ", "_")
    .replace("(", "")
...
df = df.toDF(*clean_columns)
    ...
    with tab_prep:
        ...
        **Key Steps:**
        ...
        """)
        st.code("""
# Filtering out sparse time series in Preprocessing.py
...
hier_df = hier_df.join(valid_series, on="unique_id", how="inner")
    ...
    with tab_baseline:
        ...
        **Key Steps:**
        ...
        """)
        st.code("""
# StatsForecast AutoARIMA fitment in Baseline.py
sf = StatsForecast(
...
validation_forecast = sf.forecast(df=train_df, h=12)
    ...
    with tab_reconcile:
        ...
        **Key Steps:**
        ...
        """)
        st.code("""
# Reconcile forecasts using hierarchicalforecast in Baseline.py
